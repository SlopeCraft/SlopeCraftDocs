...
打开刚刚导出文件的文件夹，将导出的投影文件移动到.minecraft[^mcdir]/schematic 文件夹下。然后打开 Minecraft，进入存档/服务器，在你想要创建地图画的地方导入这个投影。
...
结构方块的目录为.minecraft[^mcdir]/saves/<游戏存档名>/generated/minecraft/structures
...
World Edit原理图的目录为 .minecraft[^mcdir]/config/worldedit/schematics
...
打开刚刚导出文件的文件夹，将导出的地图文件移动到 .minecraft[^mcdir]/saves/<游戏存档名>/data 文件夹下。
...
[^mcdir]: 游戏存档、配置不一定直接存放于.minecraft文件夹里，具体路径与版本隔离有关。以`saves`文件夹位置为例：如果关闭版本隔离，则saves目录在`.minecraft/saves`，否则在`.minecraft/versions/<你的游戏版本>/saves`
