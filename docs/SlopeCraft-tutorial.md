...
tips：在菜单栏的 Languages 下拉菜单中你可以找到切换语言的选项。  
tips: You can find the option to switch languages in the Languages drop-down menu in the menu bar.
...
> 补充：  
> SlopeCraft从5.10版本开始，引入了新功能：“**项目池**”。你导入的每一个图片都会被程序创建为一个项目，后续的各种转换操作会为项目添加数据 。  
*~~（没用的知识增加了(=・ω・=)！）~~* 。
>
>项目池的操作逻辑和系统文件管理器类似：选中单个图片，预览或进行相应操作
>
> 简单来说，你现在可以很方便地进行批量操作了！  
**注意：同一批图像只能使用一种转换算法，风格差异大的图建议分批操作！**
...
打开刚刚导出文件的文件夹，将导出的地图文件移动到.minecraft[^ifChange2]/saves/<游戏存档名>/data 文件夹下。
