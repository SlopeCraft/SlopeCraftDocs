...
4. In macOS, you may need to specify the path of gcc manually. If you install gcc with homebrew, you can use the following parameters to specify the path of gcc.

      ```bash
      cmake -DCMAKE_C_COMPILER=/usr/local/Cellar/gcc/12.2.0/bin/gcc-12 -DCMAKE_CXX_COMPILER=/usr/local/Cellar/gcc/12.2.0/bin/g++-12 ...
      ```

      Please note that the path may be different because your gcc version is different. You can use `brew info gcc` to check the installation path of gcc. In addition, the `12` in the binary file (`gcc-12`/`g++-12`) is the major version number of your gcc, and you also need to modify it according to your gcc version.
